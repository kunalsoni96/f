          ...
          <div class="card">
            <div class="card-header card-header-primary card-header-icon" style="margin-top: -15px;">
              <div style="background-color: #04A55B; padding: 10px; padding-left: 16px; padding-top:16px; border-radius: 3px;" style="width: 100%;">
                <h4>Manage Banner
                  <a href="{{route('Admin/Banner/Add')}}" class="btn btn-success pull-right" style="margin-top: -7px; background-color: white; color:#04A55B; box-shadow:none">Add New</a></h4>
                </div>
              </div>
              <div class="card-body">
              ...
              @foreach($data as $row)
                     <tr>
                        ...
                        <td>{{$row->Title}}</td>
                        <td>
                          <img src="https://s3.ap-south-1.amazonaws.com/sanviexim/banners/{{$row->Image}}" style="width:120px;height:75px;">
                        </td>
                        <td>
                           @if($row->Status==1)
                        ...
                        <form action="{{route('Admin/Banner/EditSession')}}" method="post">
                          {{csrf_field()}}
                          ...
                          <button type="button" rel="tooltip" data-toggle="modal" data-target="#myModal" class="btn btn-danger pull-right" onclick="delete_function('{{$row->id}}')" style="background-color: #04A55B; padding:10px; padding-left: 15px; box-shadow: none; padding-right: 15px;" @if($row->id==1) disabled @endif>
                            Delete
                          </button>
                        </td>
                      </tr>
